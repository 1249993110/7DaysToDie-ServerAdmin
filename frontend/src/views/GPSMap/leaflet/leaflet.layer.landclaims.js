import { getLandClaims, removePlayerLandClaimByPosition } from '~/api/gameServer';
        ...
        for (let i = 0, len = claimOwners.length; i < len; i++) {
            ...
            for (let j = 0, len1 = claimPositions.length; j < len1; j++) {
                ...
                L.DomEvent.on(inventoryButton, 'click', async () => {
                    if (await myConfirm({ text: i18n.global.t('views.map.removeLandClaimConfirm'), icon: 'warning' })) {
                        await removePlayerLandClaimByPosition(position.x, position.y, position.z);
