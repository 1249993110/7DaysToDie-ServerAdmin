        ...
        public override void Execute(List<string> args, CommandSenderInfo senderInfo)
        {
            try
            {
                ...
                else if (args.Count == 0)
                {
                    ...
                    if (GameManager.Instance.World.Players.dict.TryGetValue(entityId, out var player) == false)
                    {
                        Log("ERR: Unable to get your position.");
                        return;
                    }

                    var playerPosition = player.GetBlockPosition();
                ...
                if (Utils.TryGetBlockValue(blockIdOrName, out var blockValue) == false)
                {
