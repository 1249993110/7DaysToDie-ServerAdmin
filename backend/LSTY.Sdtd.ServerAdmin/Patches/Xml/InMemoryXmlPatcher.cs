...
namespace LSTY.Sdtd.ServerAdmin.Patches.Xml
{
    internal static class InMemoryXmlPatcher
    {
        public const string TagPrefix = "ty_";
        public const string ActionPrefix = "act_";
        ...
        private static byte[] AttachTags(string xmlName, byte[] compressedXmlData, HashSet<string> addedTags)
        {
            ...
            foreach (XmlElement xmlElement in xmlNodeList.OfType<XmlElement>())
            {
                string itemName = xmlElement.GetAttribute("name");
                string tag = TagPrefix + itemName;

                var tagsNode = xmlElement.SelectSingleNode("property[@name='Tags']") as XmlElement;
                if (tagsNode == null)
                {
                    var newElement = xmlDocument.CreateElement("property");
                    newElement.SetAttribute("name", "Tags");

                    string[]? parentTags = GetParentNodeTags(xmlElement, rootNode, xmlNameWithoutS);
                    if (parentTags != null)
                    {
