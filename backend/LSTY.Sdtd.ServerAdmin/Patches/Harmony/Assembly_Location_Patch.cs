...
namespace LSTY.Sdtd.ServerAdmin.Patches.Harmony
{
    ...
    internal static class Assembly_Location_Patch
    {
        ...
        {
            if (string.IsNullOrEmpty(__result) && ModMain.ModInstance.ContainsAssembly(__instance))
            {
                __result = Path.Combine(ModMain.ModInstance.Path, __instance.GetName().Name + ".dll");
            }
        }
    }
}
